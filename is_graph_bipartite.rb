...
def is_bipartite(graph)
    ...
    return true if graph_len < 2
    debug = false
    
    # array to denote groups (0, 1, or nil)
    subsets = Array.new(graph_len)

    puts "graph: #{graph}" if debug

    graph.each_index { |node| 
        puts "node: #{node}, subsets: #{subsets}" if debug
        ...
        if !subsets[node]
            ...
            while stack.size > 0
                node = stack.pop
                puts "node: #{node}, stack: #{stack}" if debug
                graph[node].each { |neighbor|
                    puts "neighbor: #{neighbor}" if debug
                    if !subsets[neighbor]
                        stack.push(neighbor)
                        subsets[neighbor] = subsets[node] ^ 1
                    elsif subsets[neighbor] == subsets[node]
                        return false
                    end
                }                
            end            
